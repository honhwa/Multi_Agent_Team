...
| `packages/runtime_core/kernel_host.py` | Legacy host compatibility shell | Agent OS assembly still needs a compatibility host object during legacy-office migration | bootstrap legacy host wiring; explicit legacy host facade; health/role-lab/sandbox surfaces still observe legacy host data; host-structure and route-helper fallback categories are now drained, but low-frequency debug/helper fallbacks still transit the mixed host class | blackboard orchestration lives outside the class, entry-condition verification stays green, and remaining debug/helper fallback access is either drained or replaced by canonical runtime surfaces |
